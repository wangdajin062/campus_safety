...
# White-box GLO — most prominent (thick solid, high fill, circle markers)
ax1.plot(angles, wb, "o-", color="#1f77b4", lw=1.6, ms=5, label="White-box",
         markeredgecolor="black", markeredgewidth=0.4)
ax1.fill(angles, wb, color="#1f77b4", alpha=0.22, zorder=2)

# Black-box inversion — moderate (solid, square markers)
ax1.plot(angles, bb, "s-", color="#d62728", lw=1.4, ms=4.5, label="Black-box",
         markeredgecolor="black", markeredgewidth=0.4)
ax1.fill(angles, bb, color="#d62728", alpha=0.15, zorder=1)

# Random baseline — subdued (dotted, triangle, no fill)
ax1.plot(angles, rand, "^:", color="#7f7f7f", lw=0.9, ms=4, label="Random baseline",
...
ax1.set_ylim(0, 1.05)
ax1.set_title(" a) Privacy metrics (PIPL)",
              weight="bold", fontsize=9, pad=22)
...
ax2.scatter([wb_raw["wer"]], [wb_raw["spk_id"]], s=100, color="#1f77b4",
            edgecolor="black", lw=0.4, zorder=4, label="White-box GLO")
ax2.scatter([bb_raw["wer"]], [bb_raw["spk_id"]], s=100, marker="s",
            color="#d62728", edgecolor="black", lw=0.4, zorder=4,
            label="Black-box inversion")
ax2.scatter([rd_raw["wer"]], [rd_raw["spk_id"]], s=100, marker="^",
            color="#7f7f7f", edgecolor="black", lw=0.4, zorder=4,
            label="Random (10 spk)")
...
ax2.set_xlabel("Reconstruction WER ($\\uparrow$ = more privacy)")
ax2.set_ylabel("Speaker-ID accuracy ($\\downarrow$ = more privacy)")
